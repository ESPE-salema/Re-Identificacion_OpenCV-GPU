...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            results = [(result_silueta[0], result_silueta[1]), (result_textura[0], result_textura[1]), (result_color[0], result_color[1])]
            results = sorted(results, key=lambda x: x[1], reverse=True)
            print("resultado del convinado",results)
            if results[0][1] < 70:
                cv2.putText(img,"%: {:.2f}".format(100- results[0][1])+" "+'{}'.format(imagePaths[ results[0][0]]),(x1, y1-25),1,1.2,(255,0,0),2,cv2.LINE_AA)
